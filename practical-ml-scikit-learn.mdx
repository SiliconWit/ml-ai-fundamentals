...
```text
  ...
  2. Explore (statistics, distributions, correlations)
     │
  3. Preprocess (scale, encode, handle missing values)
     │
  4. Split (train / test, or cross-validation)
     │
  5. Train Models (fit on training data)
     │
  6. Evaluate (score on held-out data)
     │
  7. Tune Hyperparameters (GridSearchCV)
     │
  8. Select Best Model
     │
  9. Save and Deploy (joblib)
```
